...
- **GFNI**（Galois Field New Instructions）：Intel新一代指令集，专为GF(2^8)等有限域运算设计，可极大提升S盒和GCM乘法效率。
...
- **认证部分（GHASH）**：核心为GF(2^128)上的乘法，可用PCLMULQDQ、GFNI等指令集加速。
- **优化方向**：
  - 预处理多组分组并行加密，提升吞吐量。
...
- 当前SM4基础版与T-table优化版已实现并通过功能与性能测试。
- 后续将持续关注并实现基于AES-NI、GFNI、VPROLD等新指令集的高性能优化。
- 计划实现SM4-GCM模式，结合SIMD和新指令集，打造高性能、易用的国密加密认证库。
