#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// S盒
const unsigned char SBox[256] = {
    
};

// 系统参数FK
const unsigned int FK[4] = {0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC};

// 固定参数CK
const unsigned int CK[32] = {
    0x00070E15,0x1C232A31,0x383F464D,0x545B6269,
    0x70777E85,0x8C939AA1,0xA8AFB6BD,0xC4CBD2D9,
    0xE0E7EEF5,0xFC030A11,0x181F262D,0x343B4249,
    0x50575E65,0x6C737A81,0x888F969D,0xA4ABB2B9,
    0xC0C7CED5,0xDCE3EAF1,0xF8FF060D,0x141B2229,
    0x30373E45,0x4C535A61,0x686F767D,0x848B9299,
    0xA0A7AEB5,0xBCC3CAD1,0xD8DFE6ED,0xF4FB0209,
    0x10171E25,0x2C333A41,0x484F565D,0x646B7279
};

// 循环左移函数
unsigned int ROL(unsigned int data, unsigned int len) {
    return ((data << len) | (data >> (32 - len)));
}

// 线性变换函数L
unsigned int L(unsigned int X) {
    return X ^ ROL(X, 2) ^ ROL(X, 10) ^ ROL(X, 18) ^ ROL(X, 24);
}

// 线性变换函数L'
unsigned int L_prime(unsigned int X) {
    return X ^ ROL(X, 13) ^ ROL(X, 23);
}

// 非线性变换τ (S盒替换)
unsigned int tau(unsigned int A) {
    unsigned char a[4];
    unsigned int B;
    
    // 将输入拆分为4个字节
    a[0] = (A >> 24) & 0xFF;
    a[1] = (A >> 16) & 0xFF;
    a[2] = (A >> 8) & 0xFF;
    a[3] = A & 0xFF;
    
    // S盒替换
    a[0] = SBox[a[0]];
    a[1] = SBox[a[1]];
    a[2] = SBox[a[2]];
    a[3] = SBox[a[3]];
    
    // 重组为32位整数
    B = (a[0] << 24) | (a[1] << 16) | (a[2] << 8) | a[3];
    return B;
}

// T变换
unsigned int T(unsigned int X) {
    return L(tau(X));
}

// T'变换
unsigned int T_prime(unsigned int X) {
    return L_prime(tau(X));
}

// 密钥扩展函数
void KeyExpansion(unsigned int MK[], unsigned int rk[]) {
    unsigned int K[36];
    int i;
    
    // 初始化中间密钥
    K[0] = MK[0] ^ FK[0];
    K[1] = MK[1] ^ FK[1];
    K[2] = MK[2] ^ FK[2];
    K[3] = MK[3] ^ FK[3];
    
    // 生成轮密钥
    for(i = 0; i < 32; i++) {
        K[i+4] = K[i] ^ T_prime(K[i+1] ^ K[i+2] ^ K[i+3] ^ CK[i]);
        rk[i] = K[i+4];
    }
}

// 加密函数
void SM4_Encrypt(unsigned int X[], unsigned int rk[], unsigned int Y[]) {
    unsigned int state[36];
    int i;
    
    // 初始化状态
    state[0] = X[0];
    state[1] = X[1];
    state[2] = X[2];
    state[3] = X[3];
    
    // 32轮加密
    for(i = 0; i < 32; i++) {
        state[i+4] = state[i] ^ T(state[i+1] ^ state[i+2] ^ state[i+3] ^ rk[i]);
    }
    
    // 反序输出
    Y[0] = state[35];
    Y[1] = state[34];
    Y[2] = state[33];
    Y[3] = state[32];
}

// 解密函数
void SM4_Decrypt(unsigned int X[], unsigned int rk[], unsigned int Y[]) {
    unsigned int state[36];
    int i;
    
    // 初始化状态
    state[0] = X[0];
    state[1] = X[1];
    state[2] = X[2];
    state[3] = X[3];
    
    // 32轮解密（使用反序的轮密钥）
    for(i = 0; i < 32; i++) {
        state[i+4] = state[i] ^ T(state[i+1] ^ state[i+2] ^ state[i+3] ^ rk[31-i]);
    }
    
    // 反序输出
    Y[0] = state[35];
    Y[1] = state[34];
    Y[2] = state[33];
    Y[3] = state[32];
}

// 主函数示例
int main() {
    // 示例密钥
    unsigned int MK[4] = {0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210};
    // 轮密钥存储
    unsigned int rk[32];
    // 示例明文
    unsigned int plaintext[4] = {0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210};
    // 密文存储
    unsigned int ciphertext[4];
    // 解密结果存储
    unsigned int decrypted[4];
    
    // 密钥扩展
    KeyExpansion(MK, rk);
    
    // 加密
    SM4_Encrypt(plaintext, rk, ciphertext);
    
    // 解密
    SM4_Decrypt(ciphertext, rk, decrypted);
    
    // 打印结果
    printf("明文: %08X %08X %08X %08X\n", plaintext[0], plaintext[1], plaintext[2], plaintext[3]);
    printf("密文: %08X %08X %08X %08X\n", ciphertext[0], ciphertext[1], ciphertext[2], ciphertext[3]);
    printf("解密: %08X %08X %08X %08X\n", decrypted[0], decrypted[1], decrypted[2], decrypted[3]);
    
    return 0;
}
